...
SELECT `students`.*, `exams`.`course_id`, COUNT(`exam_student`.`exam_id`), MAX(`exam_student`.`vote`), ROUND(AVG(`exam_student`.`vote`), 2)
FROM `students`
...
GROUP BY `students`.`id`, `exam`.`course_id`
HAVING `appelli_d'esame` > 1;